function fit_params_out = fit_ACG(acg_narrow,plots)
    ...
    if size(acg_narrow,2)>size(acg_narrow,1)
        acg_narrow = acg_narrow';
    end
    
    % acg_narrow = cell_metrics.acg.narrow;
    midSize = ceil(length(acg_narrow)/2);
    acg_narrow(midSize-1:midSize+1) = 0; % Sets the time-zero bin to zero (-2ms -> 2ms)
    offset = midSize;
    x = ([1:midSize]/2)';
    ...
    if parallel_toolbox_installed
        ...
        gcp;
        if ~any(isnan(acg_narrow))
            [f0,gof] = fit(x,acg_narrow(x*2+offset),g,'StartPoint',a0,'Lower',lb,'Upper',ub);
            plotf0 = f0(x);
