...
% this version if for kilosrt result saved in same mother folder as data
function cluster = clusterImport(filePath,Fs,TimeStamps,p)
...
% specify file name
paramFile = fullfile(filePath, 'params.py');
spk_ClusterIndFile = fullfile(filePath,'spike_clusters.npy');
spk_TsIndFile = fullfile(filePath,'spike_times.npy');
if exist(fullfile(filePath, 'cluster_group.csv')) 
    cgsFile = fullfile(filePath, 'cluster_group.csv');
end
if exist(fullfile(filePath, 'cluster_group.tsv')) 
    cgsFile = fullfile(filePath, 'cluster_group.tsv');
end

chanMapFile = fullfile(filePath, 'channel_map.npy');
chanMap = readNPY(chanMapFile);

% load x,y coords for recording channels
coords = readNPY(fullfile(filePath, 'channel_positions.npy'));
xcoords = coords(:,1);
ycoords = coords(:,2); 
...
% load templates
temps = readNPY(fullfile(filePath, 'templates.npy'));
% load whitening mat inv
winv = readNPY(fullfile(filePath, 'whitening_mat_inv.npy'));
% load templates
spikeTemplates = readNPY(fullfile(filePath, 'spike_templates.npy')); % note: zero-indexed
% load scaling amps
tempScalingAmps = readNPY(fullfile(filePath, 'amplitudes.npy'));
[spikeAmps, ~, ~, ~, ~, ~, ~] = ...
...
% load cluster Info
cluInfo = tdfread(fullfile(filePath,'cluster_info.tsv'));

% get good clusters spike ind, time etc
goodInd = find(cgs == 2);

for k = 1:length(goodInd)
    indTemp = cids(goodInd(k));
...
%     cluster{i}.template_waveform = waveforms(templateIndTemp_2,:)*1000;
    ...
    wf = getWaveForms3(mmf,cluster{k}.spkInd(1:end-1),cluInfo.ch(ind2),p);
    ...
    TITLE1 = sprintf('%s%d%s%d%s%d','Chan-',cluster{k}.channel.channel_ID,'-Depth-',cluster{k}.channel.channel_depth,'-xCoord-',cluster{k}.channel.xcoords);
    TITLE2 = sprintf('%d%s',length(waveIdx),' spikes');
    title({TITLE1;TITLE2});
    ...
    if isfield(p,'sessInfo')
        TITLE1 = sprintf('%s%d%s%s','Rat-',p.sessInfo.ratID,'-Day-',p.sessInfo.Date);
    else
        TITLE1 = [];
    end
    ...
    text(0.5,waveMax-textPos(1),sprintf('%s%1.3f','duration: ',cluster{k}.waveMetrics.duration*10^3))
    ...
    text(0.5,waveMax-textPos(6),sprintf('%s%1.2f','isi\_violation: ',cluster{k}.waveMetrics.isi_violation))
    ...
    % plor acg narrow
    subplot(2,2,4)
    plot([-50:0.5:50],cluster{k}.waveMetrics.acg.acg_narrow,'r')
    hold on
    ...
    if p.savePlot == 1
        figure(1)
        figName = sprintf('%s%s%d%s%s%s%d%s%d%s%d',p.savedir_1,'\Rat-',p.sessInfo.ratID,'-Day-',p.sessInfo.Date,'-Depth-',cluster{k}.channel.channel_depth,'-xCoord-',cluster{k}.channel.xcoords,'-KsID-',cluster{k}.ID);
