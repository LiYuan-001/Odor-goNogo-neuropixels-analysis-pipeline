...
% 'kilosort4'
% Li Yuan, Oct-30-2025
function cluster = clusterImport_ks4(filePath,Fs,TimeStamps,p)
...
%     cluster{i}.template_waveform = waveforms(templateIndTemp_2,:)*1000;
    ...
    text(0.5,waveMax-textPos(2),sprintf('%s%1.3f','pt_ratio: ',cluster{k}.waveMetrics.pt_ratio),'Interpreter','None')
    text(0.5,waveMax-textPos(3),sprintf('%s%3.1f','amp: ',cluster{k}.waveMetrics.amp))
    % text(0.5,waveMax-textPos(5),sprintf('%s%1.2f','contam\_Ratio: ',cluster{k}.waveMetrics.acg.contamination))
    text(0.5,waveMax-textPos(4),sprintf('%s%2.2f','isi\_violation: ',cluster{k}.waveMetrics.isi_violationPct))
    ...
    if p.savePlot == 1
        figure(1)
        figName = sprintf('%s%s%s%s%s%s%d%s%d%s%d',p.savedir_1,'\Rat-',p.sessInfo.animalID,'-Date-',p.sessInfo.recDate,'-ycoord-',cluster{k}.channel.channel_depth,'-xCoord-',cluster{k}.channel.xcoords,'-KsID-',cluster{k}.ID);
