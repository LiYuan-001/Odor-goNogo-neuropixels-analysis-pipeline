function PlotMonoSyn_inh(ccgR,sig_con_inh,Pred,Bounds,cellID,cellType,cellDepth,binSize,duration,p)
...
allcel = unique(sig_con_inh(:));
%%%%%%%%%%%%%%%%%%%%%%%%%%
for i=1:length(allcel)
    prs = sig_con_inh(any(sig_con_inh==allcel(i),2),:);
...
%             plot(t,inh,'c*');
            ...
            plot(t,Bounds(:,prs1(2),prs1(1),1),'--','Color',[0.5,0.5,0.5]);
            plot(t,Bounds(:,prs1(2),prs1(1),2),'--','Color',[0.5,0.5,0.5]); 
            % Plot signif increased bins in red          
            plot(t(inh>0),inh(inh>0),'k*');
            % Plot signif lower bins in blue            
%             plot(t,inh,'c*');                     
...
%             waitfor(h)
    ...
    figName = sprintf('%s%s%d%s%s%s%d%s%d%s',p.savedir_1,'\Rat-',p.ratID,'-Day-',p.day,'-Depth-',cellDepth(allcel(i)),'-ID-',cellID(allcel(i)),'-Inh_MonoPair');
