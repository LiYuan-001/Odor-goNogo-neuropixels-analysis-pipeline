...
% 
% add bad channel index input
% Li YUAN, UCSD
 
function SGLXMetaToCoords2_badChan(badChan)
...
if bPlot
    plotSaved_badchan(xCoord, yCoord, shankInd, meta, badChan);
end
...
switch outType
    ...
    case 1     %KS2 *.mat
        ...
        connected(badChan) = 0; % added by Li Yuan
...
% Plot x z positions of all electrodes and saved channels
%
function plotSaved_badchan(xCoord, yCoord, shankInd, meta, badChan)
    % get geometry
    g = getGeomParams(meta);
    ...
    scatter( xCoord(badChan) + shankInd(badChan)*g.shankPitch, yCoord(badChan), 40, 'r', 'square', 'filled' );hold on;   
    xlim([min(xall)-5, (g.nShank-1)*g.shankPitch + max(xall)+5]);
    ylim([min(yall)-5, max(yall)+5]);
    hold off;
end % plotSaved
