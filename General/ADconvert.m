% Convert factor for AP and LFP for neuropixel recordings from SpikeGLX
% V = i * Vmax / Imax / gain
% Vmax = meta.imAiRangeMax;
% Imax = 512;
% gain is written in meta.imroTbl for each channel
% convert to Volt
% Li Yuan, 7-Jul-2022, UCSD
function converter = ADconvert(meta)
...
APgain = zeros(acqCountList(1),1);     % default type = float64
LFgain = zeros(acqCountList(2),1);     % empty array for 2.0
...
if isfield(meta,'imMaxInt')
    Imax = str2num(meta.imMaxInt);
else
    Imax = 512;
end

converter.AP = Vmax/Imax./APgain;
converter.LF = Vmax/Imax./LFgain;
