...
% -------------------------------------------------------------------------
% directly used gngGetTS_check_error_AB_ABCD from common folder
% Did not change logic except wrapped it with call function
% evnt only takes successful trials, results take all trials
% results.suc_delay = 0 for premature outcome
% LiYUAN, 2026-Mar

function [evnt,results] = gngGetTS_check_error_AB_ABCD_labView(events)
...
if sum(fTrial(1:2)<10)==2% error type A (If first two events do not include a timestamp for some reason- added an extra starting value)
    n_start=2;
    disp('txt log Error A: Immature or Mature') %Start reading events at entry #2 (assumes #1 was randomly added)   
    ErrID=1;
elseif sum(fTrial([1 7])<10)==2% no error, mature trial
    n_start=1;
    disp('txt log No Error');
    ErrID=0;
elseif fTrial(1)<10 && fTrial(7)>10
    if fTrial(1)==0% no error, premature trial
        n_start=1;disp('txt log No Error');ErrID=0;
    elseif fTrial(1)==1 %  error type 2
        n_start=6;disp('txt log Error Type2')
        ErrID=1;  
    end
else
    Error=input('txt log New Type Error');
end
