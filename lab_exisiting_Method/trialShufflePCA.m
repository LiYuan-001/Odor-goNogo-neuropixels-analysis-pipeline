function [meanFivePercentile] = trialShufflePCA(folderName,odorIndex_allCells, rst_all_allCells, Time, x_range, numPC, mode, BehavPerformance,inhStimTag,firstLast,shuffle)
...
%% make distination for saving folder
    ...
    mkdir AB_shuffle
    folder =strcat(folderName, '\AB_shuffle');
    cd (folder)
...
%         odor1000.CD(ss) =  mean(ECdist.CD1(31:50));
        ...
        if showFig ==1
           ...
           outf=['AB_PCAdist_shuffle_',num2str(m)];
           save(outf,'odor1000') 
    % if mode==0
